...
function ProjectUpdateLegendItem({ label, count, total, compact }) {
...
export default function ProjectUpdateStatusSection({
  ...
  const projectUpdateDonutSegments = useMemo(() => {
    if (!projectUpdateStatusTotal) {
      return [];
    }

    const rowsByLabel = new Map(projectUpdateStatusRows.map((row) => [row.label, row]));
    const baseSegments = PROJECT_UPDATE_STATUS_ORDER.map((label) => ({
      label,
    ...
    return baseSegments.map((segment) => {
      const rawPercent = (segment.count / projectUpdateStatusTotal) * 100;
  ...
  }, [projectUpdateStatusRows, projectUpdateStatusTotal]);
  ...
  return (
    <View className="mt-6 rounded-[18px] border border-[#dfe3ea] bg-white px-3 py-3">
      <Text className="text-[18px] uppercase tracking-[0.7px] text-[#173e8c]" style={{ fontFamily: "Montserrat-SemiBold" }}>
        Project Update Status Dashboard
      </Text>
      <Text className="mt-1 text-[13px] text-[#6b7280]" style={{ fontFamily: "Montserrat" }}>
        Risk bands based on latest project update date from SubayBAYAN records.
      </Text>
      ...
      {isLoadingSummary ? (
        <View className="items-center justify-center px-4 py-8">
          ...
          <Text className="mt-3 text-[13px] text-[#475569]" style={{ fontFamily: "Montserrat" }}>
            Loading project update status summary...
          </Text>
        </View>
      ) : summaryError ? (
        <View className="mt-3 rounded-[14px] border border-[#f4c7c7] bg-[#fff5f5] px-4 py-4">
          <Text className="text-[14px] text-[#991b1b]" style={{ fontFamily: "Montserrat-SemiBold" }}>
            Unable to load project update status summary.
          </Text>
        ...
        <View className="mt-3 w-full items-center">
              ...
              {(() => {
                ...
                return (
                  <>
                    ...
                    {projectUpdateDonutSegments.map((segment) => {
          ...
          <View className="mt-3" style={{ width: "100%", minWidth: riskLegendWidth }}>
            ...
            >
              {projectUpdateStatusRows.map((row) => (
                <ProjectUpdateLegendItem
                  key={row.label}
                  label={row.label}
                  count={row.count}
        ...
        <View className="mt-3 rounded-[14px] border border-[#e2e8f0] bg-[#f8fafc] px-4 py-4">
          <Text className="text-[13px] text-[#475569]" style={{ fontFamily: "Montserrat" }}>
            No project update status records available yet.
          </Text>
        </View>
      )}
    </View>
