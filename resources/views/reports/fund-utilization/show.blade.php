            ...
            <div style="border: 1px solid #e5e7eb; border-left: 4px solid {{ $movStatusColor }}; border-radius: 8px; margin-bottom: 18px; overflow: hidden; background-color: white;">
                ...
                <div style="padding: 12px; background-color: #f9fafb; border: 1px solid #e5e7eb; border-radius: 6px;">
                    ...
                    <form action="{{ route('fund-utilization.upload-mov', $report->project_code) }}" method="POST" enctype="multipart/form-data" style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 8px; align-items: center;">
                        ...
                        <input type="file" name="mov_file" class="dashboard-file-input" accept="application/pdf" style="flex: 1; min-width: 200px;" onchange="showSaveButton(this, 'mov-save-btn-{{ $quarter }}', 'mov-filename-{{ $quarter }}')" {{ $movUploads[$quarter] && $movUploads[$quarter]->mov_file_path && $isMovReturned ? 'disabled' : '' }} title="{{ $movUploads[$quarter] && $movUploads[$quarter]->mov_file_path && $isMovReturned ? 'Document was returned. Delete the current file to upload a new one.' : '' }}">
        ...
        <div id="dbm-notes-{{ $quarter }}" style="display: none; margin-top: 6px; padding: 6px; background-color: white; border: 1px solid #e5e7eb; border-radius: 4px;">
            ...
            <button type="button" onclick="saveRemarksAjax('dbm-secretary', '{{ $quarter }}')" style="margin-top: 4px; width: 100%; padding: 4px; background-color: #059669; color: white; border: none; border-radius: 4px; cursor: pointer; font-weight: 600; font-size: 10px;"><i class="fas fa-check" style="margin-right: 8px;"></i>Save</button>
                            ...
                            <div style="padding: 12px; background-color: {{ $dilgFieldBg }}; border: 1px solid #e5e7eb; border-radius: 6px;">
                                ...
                                @endif
                                <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 8px; align-items: center;">
                                    <input type="file" name="secretary_dilg" class="dashboard-file-input" accept="image/*,.pdf" style="flex: 1; min-width: 200px;" onchange="showSaveButton(this, 'dilg-save-btn-{{ $quarter }}', 'dilg-filename-{{ $quarter }}')" {{ $writtenNotices[$quarter] && $writtenNotices[$quarter]->secretary_dilg_path && !$isDilgReturned ? 'disabled' : '' }} title="{{ $writtenNotices[$quarter] && $writtenNotices[$quarter]->secretary_dilg_path && !$isDilgReturned ? 'File already uploaded. Delete the current file to upload a new one.' : '' }}">
...
@elseif(Auth::user()->agency === 'DILG')
            ...
            @endif
                            ...
                            <div style="padding: 12px; background-color: {{ $speakerFieldBg }}; border: 1px solid #e5e7eb; border-radius: 6px;">
                                ...
                                @endif
                                <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 8px; align-items: center;">
                                    <input type="file" name="speaker_house" class="dashboard-file-input" accept="image/*,.pdf" style="flex: 1; min-width: 200px;" onchange="showSaveButton(this, 'speaker-save-btn-{{ $quarter }}', 'speaker-filename-{{ $quarter }}')" {{ $writtenNotices[$quarter] && $writtenNotices[$quarter]->speaker_house_path && !$isSpeakerReturned ? 'disabled' : '' }} title="{{ $writtenNotices[$quarter] && $writtenNotices[$quarter]->speaker_house_path && !$isSpeakerReturned ? 'File already uploaded. Delete the current file to upload a new one.' : '' }}">
...
@elseif(Auth::user()->agency === 'DILG')
            ...
            @endif
                            ...
                            <div style="padding: 12px; background-color: {{ $presidentFieldBg }}; border: 1px solid #e5e7eb; border-radius: 6px;">
                                ...
                                @endif
                                <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 8px; align-items: center;">
                                    <input type="file" name="president_senate" class="dashboard-file-input" accept="image/*,.pdf" style="flex: 1; min-width: 200px;" onchange="showSaveButton(this, 'president-save-btn-{{ $quarter }}', 'president-filename-{{ $quarter }}')" {{ $writtenNotices[$quarter] && $writtenNotices[$quarter]->president_senate_path && !$isPresidentReturned ? 'disabled' : '' }} title="{{ $writtenNotices[$quarter] && $writtenNotices[$quarter]->president_senate_path && !$isPresidentReturned ? 'File already uploaded. Delete the current file to upload a new one.' : '' }}">
...
@elseif(Auth::user()->agency === 'DILG')
            ...
            @endif
                            ...
                            <div style="padding: 12px; background-color: {{ $houseFieldBg }}; border: 1px solid #e5e7eb; border-radius: 6px;">
                                ...
                                @endif
                                <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-bottom: 8px; align-items: center;">
                                    <input type="file" name="house_committee" class="dashboard-file-input" accept="image/*,.pdf" style="flex: 1; min-width: 200px;" onchange="showSaveButton(this, 'house-save-btn-{{ $quarter }}', 'house-filename-{{ $quarter }}')" {{ $writtenNotices[$quarter] && $writtenNotices[$quarter]->house_committee_path && !$isHouseReturned ? 'disabled' : '' }} title="{{ $writtenNotices[$quarter] && $writtenNotices[$quarter]->house_committee_path && !$isHouseReturned ? 'File already uploaded. Delete the current file to upload a new one.' : '' }}">
