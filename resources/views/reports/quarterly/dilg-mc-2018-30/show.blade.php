...
@section('content')
    ...
    <div class="content-header" style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">
        <div>
            <h1>DILG MC No. 2018-30 - {{ $office }}</h1>
            <p>Quarterly uploading for monitoring contractor compliance before commencement of road projects.</p>
        </div>
        <a href="{{ route('reports.quarterly.dilg-mc-2018-30', ['year' => $reportingYear]) }}"
            ...
            <div class="quarterly-submission-card" style="background:#fff;border:1px solid #e5e7eb;border-radius:14px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,.05);height:100%;">
                <div style="padding:16px 18px;background:#002c76;color:#fff;display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;">
                    ...
                    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;justify-content:flex-end;">
                        ...
                        @if ($canUpload)
                            <a href="{{ route('reports.quarterly.dilg-mc-2018-30.edit', ['office' => $office, 'quarter' => $quarterCode, 'year' => $reportingYear]) }}"
                ...
                <div style="padding:18px;">
                    @if ($canUpload)
                        <form method="POST"
                            action="{{ route('reports.quarterly.dilg-mc-2018-30.upload', ['office' => $office]) }}"
                            enctype="multipart/form-data"
                            id="dilg-mc-2018-30-upload-form-{{ $quarterCode }}"
                            ...
                            <div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;">
                                <label style="flex:1;min-width:220px;display:flex;align-items:center;gap:10px;padding:8px 12px;border:1px dashed #93c5fd;border-radius:10px;background:#eff6ff;color:#1e3a8a;cursor:pointer;">
                                    ...
                                    <input type="file" id="dilg-mc-2018-30-upload-{{ $quarterCode }}" name="document" accept="application/pdf" data-max-size-kb="10240"
                                        data-form-id="dilg-mc-2018-30-upload-form-{{ $quarterCode }}"
                                            ...
                                            <tr style="border-bottom:1px solid #eef2f7;vertical-align:top;">
                                                ...
                                                <td style="padding:10px 8px;text-align:center;">
                                                    @if ($statusLabel === 'Returned' && !empty($document->approval_remarks))
                                                        <button type="button"
                                                            class="dilg-mc-2018-30-remarks-trigger"
                                                ...
                                                <td style="padding:10px 8px;text-align:center;">
                                                    <details class="quarterly-action-menu">
                                                        ...
                                                        <div class="quarterly-action-panel">
                                                            <a href="{{ route('reports.quarterly.dilg-mc-2018-30.document', ['office' => $office, 'docId' => $document->id]) }}"
                                                            ...
                                                            @if ($canDelete)
                                                                <form method="POST"
                                                                    action="{{ route('reports.quarterly.dilg-mc-2018-30.delete-document', ['office' => $office, 'docId' => $document->id, 'year' => $reportingYear]) }}"
                                                            ...
                                                            @if ($canReviewThisDocument)
                                                                <div class="quarterly-action-form">
                                                                    <button type="button"
                                                                        ...
                                                                        data-decision-url="{{ route('reports.quarterly.dilg-mc-2018-30.approve', ['office' => $office, 'docId' => $document->id, 'year' => $reportingYear]) }}"
                                                            ...
                                                            @if ($status === 'returned' && ($isLguUser || $isProvincialDilgUser))
                                                                <button type="button"
                                                                    class="quarterly-action-item quarterly-action-item-reupload dilg-mc-2018-30-reupload-trigger"
                                                                    data-upload-target="dilg-mc-2018-30-upload-{{ $document->quarter }}"
    ...
    <div id="dilgMc201830UploadBackdrop" aria-hidden="true"></div>
    <div id="dilgMc201830UploadModal" role="dialog" aria-modal="true" aria-labelledby="dilgMc201830UploadTitle" aria-hidden="true">
        <div style="display:flex;flex-direction:column;height:100%;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:16px 20px;background:linear-gradient(135deg, #002C76 0%, #003d9e 100%);border-radius:12px 12px 0 0;flex-shrink:0;">
                <div>
                    <h3 id="dilgMc201830UploadTitle" style="color:white;font-size:16px;font-weight:700;margin:0;">Confirm Upload</h3>
                    <p id="dilgMc201830UploadSubtitle" style="margin:4px 0 0;color:rgba(255,255,255,0.82);font-size:12px;"></p>
                </div>
                <button type="button" id="dilgMc201830UploadClose" aria-label="Close upload confirmation" style="border:none;background:rgba(255,255,255,0.15);color:white;width:30px;height:30px;border-radius:999px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:18px;transition:background 0.2s;">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div style="padding:18px 20px;display:grid;gap:14px;">
                <div id="dilgMc201830UploadMessage" style="font-size:13px;line-height:1.6;color:#334155;"></div>
                <div style="display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap;">
                    <button type="button" id="dilgMc201830UploadCancel" style="padding:8px 14px;background:#e5e7eb;color:#111827;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">Cancel</button>
                    <button type="button" id="dilgMc201830UploadConfirm" style="padding:8px 14px;background:#002C76;color:#ffffff;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">Continue Upload</button>
                </div>
            </div>
        </div>
    </div>

    <div id="dilgMc201830DecisionBackdrop" aria-hidden="true"></div>
    <div id="dilgMc201830DecisionModal" role="dialog" aria-modal="true" aria-labelledby="dilgMc201830DecisionTitle" aria-hidden="true">
        <div style="display:flex;flex-direction:column;height:100%;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:16px 20px;background:linear-gradient(135deg, #002C76 0%, #003d9e 100%);border-radius:12px 12px 0 0;flex-shrink:0;">
                <div>
                    <h3 id="dilgMc201830DecisionTitle" style="color:white;font-size:16px;font-weight:700;margin:0;">Confirm Action</h3>
                    <p id="dilgMc201830DecisionSubtitle" style="margin:4px 0 0;color:rgba(255,255,255,0.82);font-size:12px;"></p>
                </div>
                <button type="button" id="dilgMc201830DecisionClose" aria-label="Close decision modal" style="border:none;background:rgba(255,255,255,0.15);color:white;width:30px;height:30px;border-radius:999px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:18px;transition:background 0.2s;">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <form id="dilgMc201830DecisionForm" method="POST" action="" style="padding:18px 20px;display:grid;gap:12px;">
                @csrf
                <input type="hidden" name="action" id="dilgMc201830DecisionAction" value="">
                <div style="font-size:12px;color:#475569;">Add remarks (optional for approve, required for return).</div>
                <textarea name="remarks" id="dilgMc201830DecisionRemarks" rows="3" placeholder="Add remarks..." class="quarterly-action-remarks"></textarea>
                <div style="display:flex;justify-content:flex-end;gap:10px;flex-wrap:wrap;">
                    <button type="button" id="dilgMc201830DecisionCancel" style="padding:8px 14px;background:#e5e7eb;color:#111827;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">Cancel</button>
                    <button type="submit" id="dilgMc201830DecisionSubmit" style="padding:8px 14px;background:#002C76;color:#ffffff;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">Confirm</button>
                </div>
            </form>
        </div>
    </div>

    <div id="dilgMc201830RemarksBackdrop" aria-hidden="true"></div>
    <div id="dilgMc201830RemarksModal" role="dialog" aria-modal="true" aria-labelledby="dilgMc201830RemarksTitle" aria-hidden="true">
        <div style="display:flex;flex-direction:column;height:100%;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:16px 20px;background:linear-gradient(135deg, #991b1b 0%, #dc2626 100%);border-radius:12px 12px 0 0;flex-shrink:0;">
                <div>
                    <h3 id="dilgMc201830RemarksTitle" style="color:white;font-size:16px;font-weight:700;margin:0;">Returned Remarks</h3>
                    <p id="dilgMc201830RemarksSubtitle" style="margin:4px 0 0;color:rgba(255,255,255,0.82);font-size:12px;"></p>
                </div>
                <button type="button" id="dilgMc201830RemarksClose" aria-label="Close remarks" style="border:none;background:rgba(255,255,255,0.15);color:white;width:30px;height:30px;border-radius:999px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:18px;transition:background 0.2s;">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div style="padding:18px 20px;">
                <div id="dilgMc201830RemarksBody" style="white-space:pre-wrap;font-size:12px;color:#111827;line-height:1.6;"></div>
                <div style="display:flex;justify-content:flex-end;margin-top:16px;">
                    <button type="button" id="dilgMc201830RemarksOk" style="padding:8px 14px;background:#991b1b;color:#ffffff;border:none;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">Close</button>
                </div>
            </div>
        </div>
    </div>

        <div id="dilgMc201830ActivityLogModal" role="dialog" aria-modal="true" aria-labelledby="dilgMc201830ActivityLogTitle" aria-hidden="true">
            @php
        ...
        <div style="display:flex;flex-direction:column;height:100%;">
            <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;padding:18px 24px 16px;background:linear-gradient(135deg, #002C76 0%, #003d9e 100%);border-radius:12px 12px 0 0;flex-shrink:0;">
                <div style="display:flex;align-items:center;gap:10px;">
                    ...
                    <div>
                        <h3 id="dilgMc201830ActivityLogTitle" style="color:white;font-size:16px;font-weight:700;margin:0;">Activity Logs</h3>
                        <p style="margin:4px 0 0;color:rgba(255,255,255,0.82);font-size:12px;">{{ $office }} workspace audit trail</p>
                    </div>
                </div>
                <button type="button" id="dilgMc201830ActivityLogClose" aria-label="Close activity logs" style="border:none;background:rgba(255,255,255,0.15);color:white;width:30px;height:30px;border-radius:999px;cursor:pointer;display:inline-flex;align-items:center;justify-content:center;font-size:18px;transition:background 0.2s;">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
    ...
    <div id="dilgMc201830ActivityLogBackdrop" aria-hidden="true"></div>

    <button id="dilgMc201830ActivityLogFab" type="button" aria-controls="dilgMc201830ActivityLogModal" aria-expanded="false" data-state="closed">
    ...
    <style>
        ...
        #dilgMc201830ActivityLogBackdrop {
            position: fixed;
            inset: 0;
        ...
        #dilgMc201830ActivityLogBackdrop.is-visible {
            opacity: 1;
            visibility: visible;
        }

        #dilgMc201830ActivityLogModal {
            position: fixed;
            left: 50%;
            top: 50%;
        ...
        #dilgMc201830ActivityLogModal.is-visible {
            opacity: 1;
            visibility: visible;
            transform: translate(-50%, -50%) scale(1);
        }

        #dilgMc201830DecisionBackdrop {
            position: fixed;
            inset: 0;
            background: rgba(15, 23, 42, 0.55);
            backdrop-filter: blur(4px);
        ...
        #dilgMc201830DecisionBackdrop.is-visible {
            opacity: 1;
            visibility: visible;
        }

        #dilgMc201830UploadBackdrop {
            position: fixed;
            inset: 0;
            background: rgba(15, 23, 42, 0.55);
            backdrop-filter: blur(4px);
        ...
        #dilgMc201830UploadBackdrop.is-visible {
            opacity: 1;
            visibility: visible;
        }

        #dilgMc201830UploadModal {
            position: fixed;
            left: 50%;
            top: 50%;
            transform: translate(-50%, -50%) scale(0.96);
        ...
        #dilgMc201830UploadModal.is-visible {
            opacity: 1;
            visibility: visible;
            transform: translate(-50%, -50%) scale(1);
        }

        #dilgMc201830RemarksBackdrop {
            position: fixed;
            inset: 0;
            background: rgba(15, 23, 42, 0.55);
            backdrop-filter: blur(4px);
        ...
        #dilgMc201830RemarksBackdrop.is-visible {
            opacity: 1;
            visibility: visible;
        }

        #dilgMc201830RemarksModal {
            position: fixed;
            left: 50%;
            top: 50%;
            transform: translate(-50%, -50%) scale(0.96);
        ...
        #dilgMc201830RemarksModal.is-visible {
            opacity: 1;
            visibility: visible;
            transform: translate(-50%, -50%) scale(1);
        }

        #dilgMc201830DecisionModal {
            position: fixed;
            left: 50%;
            top: 50%;
            transform: translate(-50%, -50%) scale(0.96);
        ...
        #dilgMc201830DecisionModal.is-visible {
            opacity: 1;
            visibility: visible;
            transform: translate(-50%, -50%) scale(1);
        }

        body.modal-open-dilg-mc-2018-30-logs {
            overflow: hidden;
        }

        body.modal-open-dilg-mc-2018-30-decision {
            overflow: hidden;
        }

        body.modal-open-dilg-mc-2018-30-upload {
            overflow: hidden;
        }

        body.modal-open-dilg-mc-2018-30-remarks {
            overflow: hidden;
        }

        #dilgMc201830ActivityLogFab {
            position: fixed;
            bottom: 24px;
            right: 24px;
            display: inline-flex;
        ...
        #dilgMc201830ActivityLogFab:hover {
            background-color: #003d9e;
            transform: translateY(-2px);
            box-shadow: 0 12px 24px rgba(0, 44, 118, 0.4);
        }

        #dilgMc201830ActivityLogFab:active {
            transform: translateY(0);
        }

        #dilgMc201830ActivityLogFab[data-state="open"] {
        ...
        @media (max-width: 575.98px) {
            ...
            #dilgMc201830ActivityLogFab span {
                display: none;
            }
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            document.querySelectorAll('.dilg-mc-2018-30-reupload-trigger').forEach(function (button) {
            ...
            const uploadModal = document.getElementById('dilgMc201830UploadModal');
            const uploadBackdrop = document.getElementById('dilgMc201830UploadBackdrop');
            const uploadClose = document.getElementById('dilgMc201830UploadClose');
            const uploadCancel = document.getElementById('dilgMc201830UploadCancel');
            const uploadConfirm = document.getElementById('dilgMc201830UploadConfirm');
            const uploadTitle = document.getElementById('dilgMc201830UploadTitle');
            const uploadSubtitle = document.getElementById('dilgMc201830UploadSubtitle');
            const uploadMessage = document.getElementById('dilgMc201830UploadMessage');
            ...
            function setUploadVisibility(isVisible) {
                ...
                document.body.classList.toggle('modal-open-dilg-mc-2018-30-upload', isVisible);
            ...
            document.querySelectorAll('form[id^="dilg-mc-2018-30-upload-form-"]').forEach(function (form) {
            ...
            const decisionModal = document.getElementById('dilgMc201830DecisionModal');
            const decisionBackdrop = document.getElementById('dilgMc201830DecisionBackdrop');
            const decisionClose = document.getElementById('dilgMc201830DecisionClose');
            const decisionCancel = document.getElementById('dilgMc201830DecisionCancel');
            const decisionForm = document.getElementById('dilgMc201830DecisionForm');
            const decisionActionInput = document.getElementById('dilgMc201830DecisionAction');
            const decisionRemarks = document.getElementById('dilgMc201830DecisionRemarks');
            const decisionTitle = document.getElementById('dilgMc201830DecisionTitle');
            const decisionSubtitle = document.getElementById('dilgMc201830DecisionSubtitle');
            const decisionSubmit = document.getElementById('dilgMc201830DecisionSubmit');
            ...
            function setDecisionVisibility(isVisible) {
                ...
                document.body.classList.toggle('modal-open-dilg-mc-2018-30-decision', isVisible);
            ...
            const remarksModal = document.getElementById('dilgMc201830RemarksModal');
            const remarksBackdrop = document.getElementById('dilgMc201830RemarksBackdrop');
            const remarksClose = document.getElementById('dilgMc201830RemarksClose');
            const remarksOk = document.getElementById('dilgMc201830RemarksOk');
            const remarksBody = document.getElementById('dilgMc201830RemarksBody');
            const remarksSubtitle = document.getElementById('dilgMc201830RemarksSubtitle');
            ...
            function setRemarksVisibility(isVisible) {
                ...
                document.body.classList.toggle('modal-open-dilg-mc-2018-30-remarks', isVisible);
            }

            document.querySelectorAll('.dilg-mc-2018-30-remarks-trigger').forEach(function (button) {
                button.addEventListener('click', function () {
                    if (!remarksBody) {
                        return;
                    }
            ...
            const activityLogModal = document.getElementById('dilgMc201830ActivityLogModal');
            const activityLogBackdrop = document.getElementById('dilgMc201830ActivityLogBackdrop');
            const activityLogFab = document.getElementById('dilgMc201830ActivityLogFab');
            const activityLogClose = document.getElementById('dilgMc201830ActivityLogClose');
            ...
            function setActivityLogVisibility(isVisible) {
                ...
                document.body.classList.toggle('modal-open-dilg-mc-2018-30-logs', isVisible);
