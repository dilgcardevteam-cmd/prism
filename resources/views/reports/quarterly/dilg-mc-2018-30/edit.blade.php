...
@section('title', 'DILG MC No. 2018-30 Encoding Form')
@section('page-title', 'DILG MC No. 2018-30 Encoding Form')
...
@section('content')
    @php
        ...
        $tableColumns = [
            ['key' => 'lgu_name', 'label' => 'Name of LGU', 'width' => '200px'],
            ['key' => 'contractors_required_to_inform', 'label' => 'Name of Contractors Required to Inform the Public Before Commencement of Road Projects', 'width' => '280px'],
            ['key' => 'contractors_required_to_coordinate', 'label' => 'Name of Contractors Required to Coordinate with the Local Traffic Management Group or its Equivalent Unit', 'width' => '280px'],
            ['key' => 'mode_of_information_provided', 'label' => 'Mode of Information Provided by the Contractors, Tarpaulains, Poster, Signage, Installation of Warning Signage and the Like', 'width' => '280px'],
            ['key' => 'lgu_actions', 'label' => 'Action/s Done by the LGU against those who did not inform the public before the commencement of their Road Projects and did not coordinate with the local traffic management group or its equivalent', 'width' => '310px'],
            ['key' => 'compliant', 'label' => 'COMPLIANT (YES/NO)', 'width' => '170px'],
            ['key' => 'remarks', 'label' => 'REMARKS', 'width' => '240px'],
        ];
        $columnWidthStorageKey = 'dilg-mc-2018-30-encoding-column-widths';
    @endphp
    ...
    <div class="content-header" style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;">
        <div>
            ...
            <p>Encode the quarterly template based on the uploaded DILG MC No. 2018-30 workbook format.</p>
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;">
            <a href="{{ route('reports.quarterly.dilg-mc-2018-30.show', ['office' => $office, 'year' => $reportingYear]) }}"
    ...
    <form method="POST" action="{{ route('reports.quarterly.dilg-mc-2018-30.save-encoding', ['office' => $office, 'quarter' => $quarter, 'year' => $reportingYear]) }}">
        @csrf
                ...
                <div style="display:flex;gap:8px;flex-wrap:wrap;">
                    ...
                    <details class="encoding-export-dropdown" style="position:relative;">
                        ...
                        <div style="position:absolute;right:0;top:calc(100% + 8px);min-width:160px;background:#fff;border:1px solid #fed7aa;border-radius:12px;box-shadow:0 12px 28px rgba(15,23,42,.16);padding:8px;z-index:20;">
                            <a href="{{ route('reports.quarterly.dilg-mc-2018-30.export-encoding', ['office' => $office, 'quarter' => $quarter, 'year' => $reportingYear, 'format' => 'excel']) }}"
                            ...
                            <a href="{{ route('reports.quarterly.dilg-mc-2018-30.export-encoding', ['office' => $office, 'quarter' => $quarter, 'year' => $reportingYear, 'format' => 'pdf']) }}"
                                style="display:flex;align-items:center;gap:8px;padding:10px 12px;border-radius:8px;color:#9a3412;text-decoration:none;font-size:12px;font-weight:700;">
                                <i class="fas fa-file-pdf"></i>
                                <span>PDF</span>
                            </a>
                            <a href="{{ route('reports.quarterly.dilg-mc-2018-30.export-encoding', ['office' => $office, 'quarter' => $quarter, 'year' => $reportingYear, 'format' => 'csv']) }}"
                                style="display:flex;align-items:center;gap:8px;padding:10px 12px;border-radius:8px;color:#9a3412;text-decoration:none;font-size:12px;font-weight:700;">
                        ...
                        @foreach ($formRows as $rowIndex => $row)
                            <tr class="encoding-row">
                                ...
                                <td style="padding:8px;border:1px solid #e5e7eb;">
                                    <textarea name="rows[{{ $rowIndex }}][contractors_required_to_inform]" rows="3"
                                        style="width:100%;padding:10px 12px;border:1px solid #d1d5db;border-radius:8px;background:#fff;font-size:12px;resize:vertical;">{{ old("rows.$rowIndex.contractors_required_to_inform", $row['contractors_required_to_inform'] ?? '') }}</textarea>
                                </td>
                                <td style="padding:8px;border:1px solid #e5e7eb;">
                                    <textarea name="rows[{{ $rowIndex }}][contractors_required_to_coordinate]" rows="3"
                                        style="width:100%;padding:10px 12px;border:1px solid #d1d5db;border-radius:8px;background:#fff;font-size:12px;resize:vertical;">{{ old("rows.$rowIndex.contractors_required_to_coordinate", $row['contractors_required_to_coordinate'] ?? '') }}</textarea>
                                </td>
                                <td style="padding:8px;border:1px solid #e5e7eb;">
                                    <textarea name="rows[{{ $rowIndex }}][mode_of_information_provided]" rows="3"
                                        style="width:100%;padding:10px 12px;border:1px solid #d1d5db;border-radius:8px;background:#fff;font-size:12px;resize:vertical;">{{ old("rows.$rowIndex.mode_of_information_provided", $row['mode_of_information_provided'] ?? '') }}</textarea>
                                </td>
                                <td style="padding:8px;border:1px solid #e5e7eb;">
                                    <textarea name="rows[{{ $rowIndex }}][lgu_actions]" rows="3"
                                        style="width:100%;padding:10px 12px;border:1px solid #d1d5db;border-radius:8px;background:#fff;font-size:12px;resize:vertical;">{{ old("rows.$rowIndex.lgu_actions", $row['lgu_actions'] ?? '') }}</textarea>
                                </td>
                                <td style="padding:8px;border:1px solid #e5e7eb;">
                                    <select name="rows[{{ $rowIndex }}][compliant]"
                                        style="width:100%;padding:10px 12px;border:1px solid #d1d5db;border-radius:8px;background:#fff;font-size:12px;">
                                        <option value="">Select</option>
                                        @foreach (['Yes', 'No'] as $option)
                                            <option value="{{ $option }}" @selected(old("rows.$rowIndex.compliant", $row['compliant'] ?? '') === $option)>{{ $option }}</option>
                                        @endforeach
                                    </select>
                                </td>
                                <td style="padding:8px;border:1px solid #e5e7eb;">
                                    <textarea name="rows[{{ $rowIndex }}][remarks]" rows="3"
    ...
    <template id="encoding-row-template">
        <tr class="encoding-row">
            ...
            <td style="padding:8px;border:1px solid #e5e7eb;">
                <textarea data-field="contractors_required_to_inform" rows="3"
            ...
            <td style="padding:8px;border:1px solid #e5e7eb;">
                <textarea data-field="contractors_required_to_coordinate" rows="3"
                    style="width:100%;padding:10px 12px;border:1px solid #d1d5db;border-radius:8px;background:#fff;font-size:12px;resize:vertical;"></textarea>
            </td>
            <td style="padding:8px;border:1px solid #e5e7eb;">
                <textarea data-field="mode_of_information_provided" rows="3"
                    style="width:100%;padding:10px 12px;border:1px solid #d1d5db;border-radius:8px;background:#fff;font-size:12px;resize:vertical;"></textarea>
            </td>
            <td style="padding:8px;border:1px solid #e5e7eb;">
                <textarea data-field="lgu_actions" rows="3"
                    style="width:100%;padding:10px 12px;border:1px solid #d1d5db;border-radius:8px;background:#fff;font-size:12px;resize:vertical;"></textarea>
            </td>
            <td style="padding:8px;border:1px solid #e5e7eb;">
                <select data-field="compliant"
                    style="width:100%;padding:10px 12px;border:1px solid #d1d5db;border-radius:8px;background:#fff;font-size:12px;">
                    <option value="">Select</option>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const table = document.getElementById('dilg-mc-2018-30-encoding-table');
