...
<div class="ops-detail-page">
    ...
    <div class="content-header">
        <h1>{{ $formMeta['page_heading'] }}</h1>
        <p style="margin: 6px 0 0; color: #475569; font-size: 14px;">{{ $formMeta['page_subtitle'] }}</p>
    </div>

    <div style="display:flex;justify-content:space-between;align-items:center;gap:12px;flex-wrap:wrap;margin-bottom:18px;">
        <div>
            <div style="font-size:18px;font-weight:700;color:#111827;">{{ $formMeta['project_view_heading'] }}</div>
            <div style="font-size:13px;color:#6b7280;margin-top:4px;">{{ $formMeta['project_view_description'] }}</div>
        </div>
        <a href="{{ route($formMeta['index_route']) }}" style="display:inline-flex;align-items:center;gap:6px;padding:9px 14px;background:#fff;color:#374151;border:1px solid #d1d5db;border-radius:8px;font-size:12px;font-weight:600;text-decoration:none;"><i class="fas fa-arrow-left"></i> Back to List</a>
    </div>
    ...
    <div style="margin-bottom:16px;">
        <div style="font-size:11px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.06em;margin-bottom:6px;">{{ $formMeta['submission_section_label'] ?? 'Quarterly Submission' }}</div>
        <div style="font-size:16px;font-weight:700;color:#111827;">{{ $formMeta['submission_heading'] }}</div>
        <div style="font-size:13px;color:#64748b;margin-top:4px;">{{ $formMeta['submission_description'] }}</div>
    </div>
            ...
            <div id="quarter-{{ $quarterCode }}" style="display:{{ $displayStyle }};padding:22px 24px;">
                <div style="border:1px solid #e5e7eb;border-radius:12px;padding:20px;background:{{ $hasUpload ? '#f8fafc' : '#ffffff' }};">
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:12px;flex-wrap:wrap;margin-bottom:14px;">
                        <div>
                            <div style="font-size:11px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.05em;">{{ $formMeta['report_short_label'] }}</div>
                            <div style="font-size:16px;font-weight:700;color:#111827;margin-top:6px;">{{ $formMeta['report_full_title'] }}</div>
                            <div style="font-size:12px;color:#64748b;margin-top:6px;">{{ $formMeta['acceptance_note'] }}</div>
                    ...
                    @elseif($isPendingRo)
                        <div style="margin-bottom:14px;padding:12px 14px;border:1px solid #bfdbfe;border-radius:10px;background:#eff6ff;color:#1d4ed8;font-size:13px;line-height:1.6;">Provincial validation is complete. This submission is awaiting DILG Regional Office approval.</div>
                    @elseif($isPendingPo)
                    ...
                    <form action="{{ route($formMeta['upload_route'], ['projectCode' => $project->project_code]) }}" method="POST" enctype="multipart/form-data" style="display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin:0 0 12px;">
                        @csrf
                        <input type="hidden" name="quarter" value="{{ $quarterCode }}">
                        <input type="file" name="report_file" accept=".pdf,.jpg,.jpeg,.png,application/pdf,image/jpeg,image/png" onchange="showSaveButton(this, 'rpmes-save-btn-{{ $quarterCode }}', 'rpmes-filename-{{ $quarterCode }}')" class="ops-upload-input dashboard-file-input{{ $canUpload ? '' : ' is-disabled' }}" {{ $canUpload ? '' : 'disabled' }} data-max-size-kb="10240">
                    ...
                    @elseif($isReturned)
                        <div style="margin-bottom:12px;font-size:12px;color:#b91c1c;">This submission was returned. Upload a corrected file to resubmit it into the approval flow.</div>
                    @endif

                    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:12px;margin-bottom:14px;">
                        <div style="padding:14px;border:1px solid #e5e7eb;border-radius:10px;background:white;"><div style="font-size:11px;font-weight:700;color:#6b7280;text-transform:uppercase;letter-spacing:.05em;">{{ $formMeta['deadline_card_label'] ?? 'Quarter Deadline' }}</div><div style="font-size:14px;font-weight:700;color:#111827;margin-top:8px;">{{ $quarterDeadlineDisplay !== '' ? $quarterDeadlineDisplay : 'No superadmin deadline set' }}</div><div style="font-size:11px;color:#64748b;margin-top:4px;">Configuration year: CY {{ $deadlineReportingYear }}</div></div>
                    ...
                    <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                        @if($hasUpload)
                            <a href="{{ route($formMeta['document_route'], ['projectCode' => $project->project_code, 'quarter' => $quarterCode]) }}" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;gap:6px;padding:10px 14px;background:#2563eb;color:white;border-radius:8px;text-decoration:none;font-size:12px;font-weight:700;"><i class="fas fa-eye"></i> View Report</a>
                        @endif

                        @if($canDelete)
                            <form action="{{ route($formMeta['delete_route'], ['projectCode' => $project->project_code, 'quarter' => $quarterCode]) }}" method="POST" style="margin:0;" onsubmit="return confirm('Delete the uploaded {{ $formMeta['report_short_name'] }} report for {{ $quarterLabel }}?');">
                                @csrf
                                @method('DELETE')
                        ...
                        @if(!$hasUpload)
                            <span style="font-size:12px;color:#64748b;">No {{ $formMeta['report_short_name'] }} report uploaded for {{ $quarterLabel }} yet.</span>
                        @endif
                    </div>
                </div>
            </div>
        ...
        <div class="rpmes-modal-card" onclick="event.stopPropagation()">
            <div style="padding:18px 20px;border-bottom:1px solid #e5e7eb;display:flex;justify-content:space-between;align-items:center;gap:12px;">
                <div>
                    ...
                    <div id="rpmesApprovalSubtitle" style="font-size:12px;color:#6b7280;margin-top:4px;">{{ $formMeta['report_short_name'] }} {{ $formMeta['validation_scope_label'] ?? 'quarterly validation' }}</div>
                </div>
...
<script>
    const rpmesApprovalBaseUrl = @json(route($formMeta['index_route']));
    const rpmesReportShortName = @json($formMeta['report_short_name']);
    ...
    function openRpmesApprovalModal(projectCode, quarter, action, quarterLabel) {
        ...
        subtitle.textContent = `${quarterLabel} validation for ${rpmesReportShortName}`;
        remarks.value = '';
        if (action === 'approve') {
            title.textContent = `Approve ${rpmesReportShortName}`;
            remarks.placeholder = 'Enter optional remarks for approval...';
            remarks.required = false;
            submitBtn.textContent = 'Approve';
            submitBtn.style.background = '#10b981';
        } else {
            title.textContent = `Return ${rpmesReportShortName}`;
