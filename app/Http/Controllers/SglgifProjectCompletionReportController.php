...
use App\Models\SglgifProjectCompletionReport;
use App\Models\SglgifProjectCompletionReportFile;
...
class SglgifProjectCompletionReportController extends Controller
{
    ...
    public function show(Request $request, string $projectCode)
    {
        ...
        $document = SglgifProjectCompletionReport::where('project_code', $project->project_code)->first();
        $documentFiles = SglgifProjectCompletionReportFile::where('project_code', $project->project_code)->get();
    ...
    public function save(Request $request, string $projectCode)
    {
        ...
        $document = SglgifProjectCompletionReport::firstOrNew(['project_code' => $project->project_code]);
        ...
        $folder = 'project-completion-reports/sglgif/' . Str::slug((string) $project->project_code, '_');
        $now = now();
        ...
        foreach ($this->singleUploadDocumentTypes() as $field) {
            ...
            $fileRecord = SglgifProjectCompletionReportFile::firstOrNew([
    ...
    public function uploadMultiDocument(Request $request, string $projectCode, string $documentType)
    {
        ...
        $validated = $request->validate([
            'document_file' => $this->multiUploadValidationRules($documentType),
        ]);
        ...
        $folder = 'project-completion-reports/sglgif/' . Str::slug((string) $project->project_code, '_') . '/' . Str::slug($documentType, '_');
        $now = now();
        $currentUser = Auth::user();
        $userId = $currentUser->idno ?? null;
        $isProvincialDilgUploader = $currentUser && $currentUser->isDilgUser() && !$currentUser->isRegionalOfficeAssignment();
        ...
        $fileRecord = new SglgifProjectCompletionReportFile();
    ...
    private function notifyUploadInterventionRecipients(object $project, array $documentTypes, array $routeConfig, array $scopeQuery = []): void
    {
        try {
            ...
            $documentSummary = count($documentTypes) === 1
                ...
                : number_format(count($documentTypes)) . ' SGLGIF completion-report documents';
            ...
            if ($actor->isLguScopedUser() && $targetProvince !== '') {
                ...
                $notificationService->notifyProvincialDilg(
                    ...
                    $message,
                    $url,
                    'sglgif-project-completion-upload'
                );
            ...
            Log::warning('Failed to create upload notifications (SGLGIF Project Completion Reports).', [
    ...
    public function viewDocument(Request $request, string $projectCode, string $documentType)
    {
        ...
        $document = SglgifProjectCompletionReport::where('project_code', $project->project_code)->first();
        $fileRecord = SglgifProjectCompletionReportFile::where('project_code', $project->project_code)
    ...
    private function handleDocumentValidation(Request $request, object $project, SglgifProjectCompletionReportFile $fileRecord, array $routeConfig, array $scopeQuery)
    {
        $user = Auth::user();
        $isDilg = strtoupper(trim((string) ($user->agency ?? ''))) === 'DILG';
        if (!$isDilg) {
    ...
    private function notifyLguUsersAfterRegionalApproval(
        object $project,
        string $documentType,
        SglgifProjectCompletionReportFile $fileRecord,
        string $action,
        ...
        try {
            ...
            if ($action === 'approve' && !$isRegionalOffice) {
                ...
                $notificationService->notifyRegionalDilg(
                    ...
                    $message,
                    $url,
                    substr('sglgif-project-completion-' . $documentType, 0, 100)
                );
            ...
            Log::warning('Failed to create approval notifications (SGLGIF Project Completion Reports).', [
                'project_code' => $project->project_code ?? null,
    ...
    private function parsePersistedActivityLog(string $line, string $projectCode): ?array
    {
        ...
        if (($context['module'] ?? null) !== 'sglgif_project_completion_reports') {
            return null;
        }
    ...
    private function getPersistedActivityLogs(string $projectCode): array
    {
        ...
        foreach ($logFiles as $logFile) {
            ...
            foreach ($logEntries as $logEntry) {
                ...
                if ($logEntry === '' || strpos($logEntry, '"module":"sglgif_project_completion_reports"') === false) {
                    continue;
                }
    ...
    private function logActivity(
        ...
        string $action,
        string $actionLabel,
        SglgifProjectCompletionReportFile $documentFile,
        ?string $remarks = null,
        ...
        Log::channel('upload_timestamps')->info('Document action', [
            'module' => 'sglgif_project_completion_reports',
    ...
    private function buildAccessibleSubayQuery($user)
    {
        ...
        $query->whereRaw("{$fundSourceExpression} IN ({$lfpSourcePlaceholders})", $lfpSources);
    ...
    {
        return [
            'title' => 'SGLGIF Project Completion Reports',
            'index_heading' => 'SGLGIF Project Completion Reports',
            'index_description' => 'View all accessible SGLGIF projects and open each project profile to manage completion-report submissions and validation.',
            'show_description' => 'Upload and validate SGLGIF project completion documents for this project.',
            'empty_state' => 'No accessible SGLGIF projects found.',
            'save_success_message' => 'SGLGIF project completion report documents saved successfully.',
        ];
    }
    ...
    {
        return [
            'index' => 'reports.one-time.project-completion-reports.sglgif',
            'show' => 'reports.one-time.project-completion-reports.sglgif.show',
            'document' => 'reports.one-time.project-completion-reports.sglgif.document',
            'document_file' => 'reports.one-time.project-completion-reports.sglgif.document-file',
            'save' => 'reports.one-time.project-completion-reports.sglgif.save',
            'validate' => 'reports.one-time.project-completion-reports.sglgif.validate',
            'validate_file' => 'reports.one-time.project-completion-reports.sglgif.validate-file',
            'upload_multi' => 'reports.one-time.project-completion-reports.sglgif.upload-multi',
        ];
    }
    ...
    private function multiUploadValidationRules(string $documentType): array
    {
        if ($documentType === 'geotagged_photos_path') {
            return ['required', 'file', 'mimetypes:image/jpeg', 'mimes:jpg,jpeg', 'max:15360'];
        }

        return ['required', 'file', 'mimes:pdf', 'max:15360'];
    }

    private function documentFieldMap(): array
    {
        return [
            'corrective_measures_conducted_path' => 'List of corrective measures conducted based on the DILG monitoring report',
            'final_sord_with_refund_path' => 'Final Statement of Receipts and Disbursement (SORD) with proof of refund',
            'final_swa_path' => 'Final Statement of Work Accomplishment (SWA)',
            'project_completion_report_path' => 'Project Completion Report',
            'certificate_of_completion_path' => 'Certificate of Completion',
            'certificate_of_occupancy_path' => 'Certificate of Occupancy',
            'certificate_of_turnover_and_acceptance_path' => 'Certificate of Turnover and Acceptance (if by Contractor)',
            'warranty_certificate_path' => '12 months warranty certificate signed by the Contractor',
            'geotagged_photos_path' => 'High-resolution geo-tagged JPEG photos of the completed project',
            'copy_of_or_cr_under_lgu_name_path' => 'Copy of OR/CR under the name of LGU for purchase of brand-new motor vehicle',
            'copy_of_official_receipt_heavy_equipment_path' => 'Copy of Official Receipt of the Heavy Equipment',
        ];
    }
