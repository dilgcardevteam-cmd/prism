...
use App\Models\QuarterlyDilgMc201830Encoding;
use App\Models\QuarterlyDilgMc201830Upload;
use App\Models\User;
...
class QuarterlyDilgMc201830Controller extends Controller
{
    public function __construct()
    {
        $this->middleware('auth');
        $this->middleware('crud_permission:quarterly_dilg_mc_2018_30,view')->only(['index', 'show', 'viewDocument']);
        $this->middleware('crud_permission:quarterly_dilg_mc_2018_30,add')->only(['edit', 'saveEncoding', 'exportEncoding', 'upload']);
        $this->middleware('crud_permission:quarterly_dilg_mc_2018_30,update')->only(['approveDocument']);
        $this->middleware('crud_permission:quarterly_dilg_mc_2018_30,delete')->only(['deleteDocument']);
    }
    ...
    public function index(Request $request)
    {
        ...
        if (!empty($missingTables)) {
            ...
            $documents = QuarterlyDilgMc201830Upload::query()
        ...
        return view('reports.quarterly.dilg-mc-2018-30.index', compact(
            'documentsByOffice',
            'filterOptions',
            'filters',
            'officeRows',
    ...
    public function edit(Request $request, string $office, string $quarter)
    {
        ...
        abort_unless(
            $user
            && $user->hasCrudPermission('quarterly_dilg_mc_2018_30', 'add')
        ...
        $encoding = QuarterlyDilgMc201830Encoding::query()
            ->where('office', $office)
            ->where('year', $reportingYear)
        ...
        return view('reports.quarterly.dilg-mc-2018-30.edit', [
            'encoding' => $encoding,
            'formRows' => $formRows,
            'office' => $office,
            'province' => $province,
    ...
    public function saveEncoding(Request $request, string $office, string $quarter)
    {
        ...
        $validated = $request->validate([
            ...
            'rows.*.contractors_required_to_inform' => ['nullable', 'string', 'max:1000'],
            'rows.*.contractors_required_to_coordinate' => ['nullable', 'string', 'max:1000'],
            'rows.*.mode_of_information_provided' => ['nullable', 'string', 'max:1000'],
            'rows.*.lgu_actions' => ['nullable', 'string', 'max:1500'],
            'rows.*.compliant' => ['nullable', 'in:Yes,No'],
        ...
        QuarterlyDilgMc201830Encoding::query()->updateOrCreate(
            [
                'office' => $office,
        ...
        return redirect()
            ->route('reports.quarterly.dilg-mc-2018-30.edit', [
                'office' => $office,
                'quarter' => $normalizedQuarter,
                'year' => (int) $validated['year'],
            ])
    ...
    public function exportEncoding(Request $request, string $office, string $quarter)
    {
        ...
        $baseFilename = 'dilg_mc_2018_30_' . Str::slug($office, '_') . '_' . strtolower($normalizedQuarter) . '_' . $reportingYear;
        if ($format === 'csv') {
        ...
        if ($format === 'pdf') {
            ...
            $pdf->SetCreator('PDMU');
            $pdf->SetAuthor('PDMU');
            $pdf->SetTitle('DILG MC No. 2018-30');
    ...
    public function upload(Request $request, string $office)
    {
        ...
        $path = $request->file('document')->store(
            'quarterly/dilg-mc-2018-30/' . Str::slug($office, '_'),
            'public'
        );
        $uploadedAt = now();
        ...
        $existingDocumentQuery = QuarterlyDilgMc201830Upload::query()
            ->where('office', $office)
        ...
        if ($existingDocument && $existingDocument->status === 'returned') {
            ...
            QuarterlyDilgMc201830Upload::query()->create([
                'office' => $office,
                'province' => $province,
                'year' => (int) $validated['year'],
        ...
        return redirect()
            ->route('reports.quarterly.dilg-mc-2018-30.show', [
                'office' => $office,
                'year' => (int) $validated['year'],
            ])
    ...
    public function viewDocument(Request $request, string $office, int $docId)
    {
        ...
        $document = QuarterlyDilgMc201830Upload::query()
            ->where('office', $office)
            ->where('id', $docId)
            ->firstOrFail();
    ...
    public function approveDocument(Request $request, string $office, int $docId)
    {
        ...
        return redirect()
            ->route('reports.quarterly.dilg-mc-2018-30.show', ['office' => $office, 'year' => $this->resolveReportingYear($request)])
    ...
    private function logDocumentActivity(Request $request, string $action, QuarterlyDilgMc201830Upload $document, ?string $remarks = null): void
    {
    ...
    private function normalizeDocumentStatus(?QuarterlyDilgMc201830Upload $document): string
    {
    ...
    private function renderDetailView(Request $request, string $office)
    {
        ...
        $canUpload = (bool) ($user
            ...
            && !$user->isRegionalOfficeAssignment());
        $canDelete = (bool) ($user
            && $user->hasCrudPermission('quarterly_dilg_mc_2018_30', 'delete')
            && $user->isDilgUser());
        $canValidate = (bool) ($user
            && $user->hasCrudPermission('quarterly_dilg_mc_2018_30', 'update')
            && $user->isDilgUser());

        return view('reports.quarterly.dilg-mc-2018-30.show', compact(
            'canDelete',
            'canUpload',
            'canValidate',
            'documentsByQuarter',
    ...
    {
        $requiredTables = ['quarterly_dilg_mc_2018_30_uploads'];

        if ($includeEncoding) {
            $requiredTables[] = 'quarterly_dilg_mc_2018_30_encodings';
        }
    ...
    {
        return 'DILG MC No. 2018-30 setup is incomplete on this deployment. Missing database tables: '
    ...
    private function buildActivityLogs(string $office, int $reportingYear): array
    {
        ...
        $logs = ActivityLog::query()
            ...
            ->where(function ($query) {
                $query
                    ->where('properties->route_name', 'like', 'reports.quarterly.dilg-mc-2018-30%')
                    ->orWhere('properties->path', 'like', '/reports/quarterly/dilg-mc-2018-30%');
            })
    ...
    private function loadEncodingRowsForExport(string $office, int $reportingYear, string $quarter): array
    {
        ...
        return $rows->map(function (array $row): array {
            return [
                $row['lgu_name'] ?? '',
                $row['contractors_required_to_inform'] ?? '',
                $row['contractors_required_to_coordinate'] ?? '',
                $row['mode_of_information_provided'] ?? '',
                $row['lgu_actions'] ?? '',
                $row['compliant'] ?? '',
                $row['remarks'] ?? '',
            ];
    ...
    private function encodingExportHeaders(): array
    {
        return [
            'Name of LGU',
            'Name of Contractors Required to Inform the Public Before Commencement of Road Projects',
            'Name of Contractors Required to Coordinate with the Local Traffic Management Group or its Equivalent Unit',
            'Mode of Information Provided by the Contractors, Tarpaulains, Poster, Signage, Installation of Warning Signage and the Like',
            'Action/s Done by the LGU against those who did not inform the public before the commencement of their Road Projects and did not coordinate with the local traffic management group or its equivalent',
            'COMPLIANT (YES/NO)',
            'REMARKS',
        ];
    }
    ...
    private function blankEncodingRow(string $office): array
    {
        return [
            'lgu_name' => $office,
            'contractors_required_to_inform' => '',
            'contractors_required_to_coordinate' => '',
            'mode_of_information_provided' => '',
            'lgu_actions' => '',
            'compliant' => '',
            'remarks' => '',
        ];
    ...
    private function normalizeEncodingRow(mixed $row, string $office): array
    {
        ...
        return [
            'lgu_name' => $office,
            'contractors_required_to_inform' => trim((string) ($data['contractors_required_to_inform'] ?? '')),
            'contractors_required_to_coordinate' => trim((string) ($data['contractors_required_to_coordinate'] ?? '')),
            'mode_of_information_provided' => trim((string) ($data['mode_of_information_provided'] ?? '')),
            'lgu_actions' => trim((string) ($data['lgu_actions'] ?? '')),
            'compliant' => trim((string) ($data['compliant'] ?? '')),
    ...
    private function sanitizeEncodingRow(array $row, string $office): array
    {
        return [
            'lgu_name' => $office,
            'contractors_required_to_inform' => InputSanitizer::sanitizePlainText($row['contractors_required_to_inform'] ?? null, true),
            'contractors_required_to_coordinate' => InputSanitizer::sanitizePlainText($row['contractors_required_to_coordinate'] ?? null, true),
            'mode_of_information_provided' => InputSanitizer::sanitizePlainText($row['mode_of_information_provided'] ?? null, true),
            'lgu_actions' => InputSanitizer::sanitizePlainText($row['lgu_actions'] ?? null, true),
            'compliant' => $this->normalizeEncodingOption($row['compliant'] ?? null, ['Yes', 'No']),
